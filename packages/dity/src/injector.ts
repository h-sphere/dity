...
export type UnwrapConfiguration<T> = Prettify<{
	[K in keyof T]: T[K] extends Configuration<infer U>
		? Awaited<U>
		: Awaited<T[K]>
}>
...
export type GetAllContainerTypes<T> =
	T extends Submodule<infer A, infer B, any> ? A & B : never

export function makeInjector<
	const B,
	const T extends 'factory' | 'class' = 'class'
>() {
	type Deps = GetAllContainerTypes<B>

	return function injector<const Inject extends Array<keyof Deps & string>>(
		config: Inject
	) {
