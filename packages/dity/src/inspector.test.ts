...
describe('Inspector', () => {
	...
	it.skip('should have 2 submodules refering each other', async () => {
		const modA = buildContainer(c =>
			c.externals<{ modAA: string; modAB: string }>()
		)
		const modB = buildContainer(c =>
			c.externals<{ modBA: string; modBB: string }>()
		)
		const mod = buildContainer(c =>
			c.submodules({ modA, modB }).register({ a: 'hello' })
		)
			.resolve({
