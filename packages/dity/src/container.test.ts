...
describe('Container', () => {
    ...
    it('should properly resolve class injections', async () => {
        const mod = buildContainer(c => c.register({
            a: 5,
            b: 10,
            c: asClass(SumClass)
        }))
    ...
    it('should properly resolve submodules', async () => {
        const helperMod = buildContainer(c => c.register({
            date: () => 66436,
            env: 'production'
        }))
    ...
    it('should resolve parent types', async () => {
        const helperModule = buildContainer(c => c.register({
            helperA: 5,
            helperB: 10,
        ...
        const timerModule = buildContainer(c => c.register({
            timerA: 100,
            timerB: 2000
        }))

        const mainModule = buildContainer(c => c.submodules({
            helper: helperModule,
            timer: timerModule
        }))
    ...
    it('should resolve multiple nested modules', async () => {
        ...
        const c = buildContainer(c => c.submodules({ b }).submodules({ f }))
        const d = buildContainer(con => con.register({ abc: 'hello' }).submodules({ c }))
