...
type AnyPromise<T extends readonly unknown[]> = Promise<any> extends T[number] ?
   true : false

type WrapInPromiseIfNeeded<T, TT extends Array<any>> = T extends Promise<any> ? T : AnyPromise<TT> extends true ? Promise<T> : T
...
const isRetFn = (x: any): x is RetFn<any> => {
    return typeof x === 'object' && ('fn' in x) && (typeof x['fn'] === 'function') && ('args' in x) && Array.isArray(x['args'])
}
...
class Inj<D extends Deps, Arr extends Array<any>, Ret> {
    ...
    inject<Keys extends TypesToKeys<D, Arr>>(...args: Keys): RetFn<WrapInPromiseIfNeeded<Ret, Values<D, Keys>>> {
        let v: Ret | null = null
...
class DepConfigurator<D extends Deps> {
    ...
    cls<C extends new (...args: any[]) => any>(Cls: C) {
        ...
        return new Inj<D, ConstructorParameters<typeof Cls>, InstanceType<typeof Cls>>(factory)
    }
    ...
    factory<C extends (new (...args: any[]) =>  { make(): any })>(Cls: C) {
        ...
        return new Inj<D, ConstructorParameters<typeof Cls>, ReturnType<InstanceType<typeof Cls>['make']>>(factory)
    }
...
export class Registrator<D extends Deps = {}, Externals extends keyof D = never, Exports extends string = never, OutsideExternals extends Deps = {}> {
    ...
    import<K extends string, T>(): Registrator<D & Record<K, T>, Externals | K, Exports, OutsideExternals> {
        return this
    }

    export<K extends (keyof D & string)[]>(...k: K): Registrator<D, Externals, Exports | K[number], OutsideExternals> {
        return this
    }

    link<
        K extends Externals | keyof OutsideExternals,
        TYPE extends K extends Externals ? D[K] : K extends keyof OutsideExternals ? OutsideExternals[K] : never
    >(k: K, e: Exclude<KeysExtendingType<D, TYPE>, K>): Registrator<D, Exclude<Externals, K>, Exports, Omit<OutsideExternals, K>> {
        const v = {
            args: [],
            fn: (d) => d.get(e)
        } satisfies RetFn<any>
    ...
    resolve<
        K extends Externals | keyof OutsideExternals,
        TYPE extends K extends Externals ? D[K] : K extends keyof OutsideExternals ? OutsideExternals[K] : never
    >(k: K, v: RetFn<TYPE> | ((d: DepConfigurator<D>) => RetFn<TYPE>)): Registrator<D, Exclude<Externals, K>, Exports, Omit<OutsideExternals, K>> {
    ...
    register<const K extends string, F extends number | string | Promise<any>>(key: K, f: F): Registrator<D & Record<K, F>, Externals, Exports, OutsideExternals>
    register<const K extends string, F extends ((d: DepConfigurator<D>) => RetFn<any>)>(key: K, f: F): Registrator<D & Record<K, FromRet<ReturnType<F>>>, Externals,
    Exports, OutsideExternals
    >
    register<const K extends string, Ret, F extends ((d: DepConfigurator<D>) => RetFn<Ret>) | number | string | Promise<any>>(key: K, f: F) {
        if (typeof f === 'number' || typeof f === 'string' || f instanceof Promise) {
    ...
    registerMany<const M extends Record<string, number | string | Promise<any> | ((d: DepConfigurator<D>) => RetFn<any>)>>(r: M): (
        Registrator<D & FromRetAll<M>, Exports, Exports, OutsideExternals>
    ) {
    ...
    >(name: M, mod: Registrator<MD, Ext, MExports, MOutsideExternals>): Registrator<
