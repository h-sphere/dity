...
class Inspect {
	...
	getDependencies(chain: ChainDef[] = []) {
		...
		const subs = this.getSubDependencies(chain) as ReturnType<
			typeof this.getSelfDepsDescriptions
		>
		return [...self, ...subs]
	}

	getSubDependencies(chain: ChainDef[] = []) {
		return Object.entries(this.#submodules).flatMap(([key, value]) => {
			let currentChain: ChainDef[] = [...chain]
			const deps = value.getDependencies([
				...currentChain,
				{ key: this.#moduleKey, name: key }
			])
			return deps.map(d => {
				if (d.type === 'reference') {
