...
export class Container<D extends Dependencies> {
	...
	constructor(
		deps: Dependencies,
		submodules: Record<string, Submodule<any, any, any>>,
		name: symbol = Symbol()
	) {
	...
	private transformReferences(v: unknown) {
		...
		if (
			...
			return {
				...
				[MODULE_KEY]:
					MODULE_KEY in v &&
					typeof v[MODULE_KEY] === 'symbol' &&
					v[MODULE_KEY] !== INJECTOR_PLACEHOLDER
						? v[MODULE_KEY]
						: this.#currentModule
			} satisfies DependencyInfo
		}
	...
	private getModule(k: string): Container<any> {
		...
		if (k in this.#submoduleDefinitions) {
			...
			const externalEntries = Object.entries(this.#dependencyDefinitions)
	...
	private async getDependencyInstance<const K extends keyof D & string>(
		key: K,
		parentChain: Container<any>[],
		resolutionChain: string[] = []
	): Promise<D[K]> {
		// Create fully qualified key for resolution tracking
		const modulePrefix = this.#currentModule.description ?? 'unknown'
		const fullyQualifiedKey =
			modulePrefix === 'toplevel' ? key : `${modulePrefix}.${key}`
		...
		if (key in this.#dependencyDefinitions) {
			...
			if (isDependencyReference(dep)) {
				return this.getWithModule(
					this.transformReferences(dep) as DependencyInfo,
					parentChain,
					[...resolutionChain, fullyQualifiedKey]
				) as D[K]
				...
				for (const d of dep.deps) {
					deps.push(
						await this.getWithModule(
							this.transformReferences(d) as DependencyInfo,
							parentChain,
							newChain
						)
					)
				...
				const res: Dependencies[K] = await this.getWithModule(
					this.transformReferences(dep) as DependencyInfo,
					parentChain,
					[...resolutionChain, fullyQualifiedKey]
				)
		...
		return this.getModule(module).getWithModule(
			subKey,
			[this, ...parentChain],
			[...resolutionChain, fullyQualifiedKey]
		)
	...
	private async getWithModule<const K extends keyof D & string>(
		conf: K | DependencyInfo,
		parentChain: Container<any>[] = [],
		resolutionChain: string[] = []
	): Promise<D[K]> {
		let { module, key } =
		...
		if (module === this.#currentModule) {
			return this.getDependencyInstance(
				key,
				parentChain,
				resolutionChain
			) as D[K]
		...
		if (matchingParent) {
			...
			const parentIndex = parentChain.findIndex(
				p => p.#currentModule === module
			)
			const trimmedChain =
				parentIndex >= 0 ? parentChain.slice(0, parentIndex) : parentChain
	...
	[getInternals]() {
		return {
			...
			submodules: Object.fromEntries(
				Object.keys(this.#submoduleDefinitions).map(key => [
					key,
					this.getModule(key)
				])
			),
